...
-- Add foreign key constraint for ai_suggestion_id (only if it doesn't exist)
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Users can read own interaction logs" ON interaction_logs;
DROP POLICY IF EXISTS "Users can insert own interaction logs" ON interaction_logs;
DROP POLICY IF EXISTS "Users can update own interaction logs" ON interaction_logs;
DROP POLICY IF EXISTS "Users can delete own interaction logs" ON interaction_logs;

DROP POLICY IF EXISTS "Users can read own ai suggestions" ON ai_suggestions;
